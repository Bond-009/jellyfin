...
namespace MediaBrowser.XbmcMetadata.Providers
{
    ...
    public class ArtistNfoProvider : BaseNfoProvider<MusicArtist>
    {
        ...
        {
            new BaseNfoParser<MusicArtist>(_logger, _config, _providerManager, _userManager, _userDataManager, FileSystem).Fetch(result, path, cancellationToken);
        }

        /// <inheritdoc />
        protected override FileSystemMetadata? GetXmlFile(ItemInfo info)
            => FileSystem.GetFileInfo(Path.Combine(info.Path, "artist.nfo"));
    }
}
